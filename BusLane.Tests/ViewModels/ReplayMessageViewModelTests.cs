...
{
    [Fact]
    public void EditingBodyAfterPreview_InvalidatesPreview()
    {
        // Arrange
        ...
        // Assert
        sut.HasPreview.Should().BeFalse();
    }

    [Fact]
    public void EditingCustomPropertyAfterPreview_InvalidatesPreview()
    {
        // Arrange
        var replayService = Substitute.For<IMessageReplayService>();
        var source = CreateSource();
    ...
    [Fact]
    public void BuildPreview_WithDuplicatePropertyKeys_ShowsValidationError()
    {
        // Arrange
        var replayService = Substitute.For<IMessageReplayService>();
        var source = CreateSource();
        ...
        var sut = new ReplayMessageViewModel(
            ...
            replayService,
            () => Substitute.For<IServiceBusOperations>());
        sut.AddCustomPropertyCommand.Execute(null);
        sut.CustomProperties[^1].Key = "tenant";
        ...
        // Assert
        sut.HasPreview.Should().BeFalse();
        sut.ErrorMessage.Should().Be("Application property keys must be unique");
        replayService.DidNotReceive().Preview(Arg.Any<ReplayRequest>());
    }

    [Fact]
    public void BuildPreview_WhenServiceValidationFails_DoesNotMarkPreviewReady()
    {
        // Arrange
        var replayService = Substitute.For<IMessageReplayService>();
        var source = CreateSource();
        var destination = CreateDestination();
        replayService.CreateRequest(source, destination).Returns(CreateRequest(source, destination));
        replayService.Preview(Arg.Any<ReplayRequest>()).Returns(
            new ReplayPreview(["Message body is required"], [], []));
        var sut = new ReplayMessageViewModel(
            source,
            [destination],
            replayService,
    ...
    [Fact]
    public async Task ReplayAsync_AfterResult_InvokesCompletionCallback()
    {
        // Arrange
        ...
        replayService.CreateRequest(source, destination).Returns(CreateRequest(source, destination));
        replayService.Preview(Arg.Any<ReplayRequest>()).Returns(new ReplayPreview([], [], []));
        replayService.ReplayAsync(operations, Arg.Any<ReplayRequest>(), Arg.Any<CancellationToken>())
